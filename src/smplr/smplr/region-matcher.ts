...
export class RegionMatcher {
  _groups: ProcessedGroup[];
  _seqCounters: Map<number, number>; // groupIndex → call count

  constructor(json: SmplrPreset) {
    this._groups = json.groups.map(processGroup);
  ...
  match(
    ...
    for (let gi = 0; gi < this._groups.length; gi++) {
      ...
      const counter = this._seqCounters.get(gi) ?? 0;
      ...
      if (group.seqLength !== undefined) {
        this._seqCounters.set(gi, counter + 1);
      }
    }
