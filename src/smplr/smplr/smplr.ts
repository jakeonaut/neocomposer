...
export class SmplrImpl implements Smplr {
  ...
  _loadProgress: LoadProgress = { loaded: 0, total: 0 };
  _loadToken = 0;
  _buffers: Map<string, AudioBuffer> = new Map();
  _reversedBuffers: Map<string, AudioBuffer> = new Map();
  _defaults: PlaybackParams | undefined;
  _defaultVelocity: number;
  _aliases: Map<string, number> | undefined;
  _matcher: RegionMatcher;
  _voices: VoiceManager;
  _channel: Channel;
  _onLoadProgress: ((progress: LoadProgress) => void) | undefined;
  _onStart: ((event: NoteEvent) => void) | undefined;
  _onEnded: ((event: NoteEvent) => void) | undefined;
  _ccState: Map<number, number> = new Map();
  _disposed = false;

  _assertNotDisposed(action: string): void {
    if (this._disposed) {
  ...
  constructor(
    ...
    this._defaultVelocity = options?.velocity ?? 100;
    this._onLoadProgress = options?.onLoadProgress;
    this._onStart = options?.onStart;
    this._onEnded = options?.onEnded;

    if (json?.aliases) {
      this._aliases = new Map(Object.entries(json.aliases));
    }

    // 1. Output channel — volume, routing, effects
    this._channel = new Channel(context, {
    ...
    this._matcher = new RegionMatcher(json ?? EMPTY_JSON);

    // 4. Voice manager — tracks active voices for stop operations
    this._voices = new VoiceManager();
  ...
  loadInstrument(
    ...
    this._assertNotDisposed("load an instrument");
    ...
    return this.loader
      ...
      .then((newBuffers) => {
        ...
        this._aliases = json.aliases
          ...
          : undefined;
        this._matcher = new RegionMatcher(json);
        this._reversedBuffers = new Map();
  ...
  setCC(cc: number, value: number): void {
    this._assertNotDisposed("set CC");
    this._ccState.set(cc, value);
  }
  ...
  getCC(cc: number): number {
    this._assertNotDisposed("read CC");
    return this._ccState.get(cc) ?? 0;
  }
  ...
  setDetune(cents: number): void {
    this._assertNotDisposed("set detune");
    if (!this._defaults) this._defaults = {};
    this._defaults.detune = cents;
  }
  ...
  setReverse(reverse: boolean): void {
    this._assertNotDisposed("set reverse");
    if (!this._defaults) this._defaults = {};
    this._defaults.reverse = reverse;
  }
  ...
  start(event: NoteEvent): StopFn {
    this._assertNotDisposed("start a note");
    const normalized = this._normalizeNoteEvent(event);

    const schedulerStop = this.scheduler.schedule(
      normalized as NoteEvent,
      (e) => this._playNote(e as NormalizedNoteEvent),
    );

    return (time?: number) => {
      schedulerStop(); // cancel from queue if not yet dispatched
      this._voices.stopById(normalized.stopId, time); // stop if already playing
    };
  }
  ...
  stop(target?: StopTarget): void {
    this._assertNotDisposed("stop voices");
    if (target === undefined) {
      this._voices.stopAll();
    } else if (typeof target === "string" || typeof target === "number") {
      this._voices.stopById(target);
    } else {
      if (target.stopId !== undefined) {
        this._voices.stopById(target.stopId, target.time);
      } else {
        this._voices.stopAll(target.time);
      }
    }
  }
  ...
  _getBuffer(sample: string, reverse: boolean): AudioBuffer | undefined {
    if (!reverse) return this._buffers.get(sample);
    const cached = this._reversedBuffers.get(sample);
    if (cached) return cached;
    const original = this._buffers.get(sample);
    ...
    this._reversedBuffers.set(sample, reversed);
    return reversed;
  }

  _playNote(event: NormalizedNoteEvent): void {
    const {
      midi,
      velocity,
      time,
    ...
    const matches = this._matcher.match(midi, velocity, this._ccState);

    // Stop exclusive groups before starting new voices
    for (const match of matches) {
      if (match.offBy !== undefined) {
        this._voices.stopGroup(match.offBy, time);
      }
    }

    // Create a voice for each matched region
    let voiceStarted = false;
    const effectiveReverse = reverse ?? this._defaults?.reverse ?? false;
    for (const match of matches) {
      const buffer = this._getBuffer(match.sample, effectiveReverse);
      if (!buffer) continue;
  ...
  _normalizeNoteEvent(event: NoteEvent): NormalizedNoteEvent {
    if (typeof event === "string" || typeof event === "number") {
      const midi = toMidi(event) ?? this._aliases?.get(String(event)) ?? 0;
      return {
        note: event,
        midi,
        velocity: this._defaultVelocity,
        stopId: event,
    ...
    const midi =
      toMidi(event.note) ?? this._aliases?.get(String(event.note)) ?? 0;
    return {
      ...event,
      midi,
      velocity: event.velocity ?? this._defaultVelocity,
      stopId: event.stopId ?? event.note,
      onStart: compose(this._onStart, event.onStart),
      onEnded: compose(this._onEnded, event.onEnded),
    };
  }
}
