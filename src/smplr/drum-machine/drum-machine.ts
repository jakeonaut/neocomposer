...
export class DrumMachine {
  ...
  getSampleNames(): string[] {
    return this._instrument.samples.slice();
  }

  getGroupNames(): string[] {
    return this._instrument.groupNames.slice();
  }

  getSampleNamesForGroup(groupName: string): string[] {
    return this._instrument.sampleGroupVariations[groupName] ?? [];
  }

  start(sample: SampleStart) {
    const sampleName = this._instrument.nameToSampleName[sample.note];
