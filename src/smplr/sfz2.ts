...
class Scope {
  ...
  closeScope(mode: string, group: RegionGroup) {
    if (mode === "global") {
      ...
      this._closeRegion(this.global as SampleRegion);
    } else if (mode === "group") {
      // Save group properties
      this.group = this._closeRegion({} as SampleRegion);
    } else if (mode === "region") {
      const region = this._closeRegion({
        sampleName: "",
        midiPitch: -1,
        ...this.global,
        ...this.group,
  ...
  _closeRegion(region: SampleRegion) {
